...
function LogPageInner() {
    ...
    if (existing) {
      ...
      setExerciseLogs(
        day.exercises.map((ex) => ({
          ...
          isCustom: false,
          isTimed: !!(ex.duration && !ex.reps),
          sets: Array.from({ length: ex.sets }, (_, i) => ({
            setNumber: i + 1, weightKg: undefined, reps: undefined, durationSeconds: undefined,
          })),
          notes: "",
        }))
      )
  ...
  function handleDayOverride(newKey: DayKey) {
    ...
    setSkippedIds(new Set())
    const day = WORKOUT_PLAN[newKey]
    const defaultLogs: ExerciseLog[] = day.exercises.map((ex) => ({
      exerciseId: ex.id, exerciseName: ex.name, isCustom: false,
      isTimed: !!(ex.duration && !ex.reps),
      sets: Array.from({ length: ex.sets }, (_, i) => ({
        setNumber: i + 1, weightKg: undefined, reps: undefined, durationSeconds: undefined,
      })),
  ...
  function handleSave() {
    try {
      ...
      // Confetti on new personal best weight for any exercise
  ...
  const workout = WORKOUT_PLAN[effectiveDayKey]
  const isRestDay = effectiveDayKey === "sunday"
    ...
    <div style={{ paddingTop: 24, paddingBottom: 8 }}>
      ...
      <div style={{ marginBottom: 20 }}>
        ...
        <div style={{
          ...
          <input type="date" value={date} max={todayStrPH()}
            onChange={(e) => setDate(e.target.value)}
            style={{ flex: 1, background: "none", border: "none", color: "var(--text-primary)", fontSize: 14, fontFamily: "inherit", outline: "none", cursor: "pointer" }} />
        </div>
      </div>
      ...
      <div style={{
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
            ...
            {isOverridden && (
              <span style={{ fontSize: 10, color: "var(--accent)", background: "var(--accent-dim)", border: "0.5px solid var(--accent-border)", borderRadius: 20, padding: "1px 8px" }}>
                Changed
              </span>
            )}
          </div>
      ...
      {isRestDay ? (
        <div style={{
          ...
          <button onClick={() => setShowDayPicker(true)}
            style={{
              marginTop: 16, background: "none", border: "0.5px solid var(--border-default)",
              borderRadius: "var(--radius-md)", color: "var(--text-secondary)",
              fontSize: 13, padding: "9px 20px", cursor: "pointer", fontFamily: "inherit",
            }}>
        ...
        <>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            ...
            {showAddForm ? (
              <AddExerciseForm
                onAdd={(ex) => { setExerciseLogs(prev => [...prev, ex]); setShowAddForm(false) }}
                onCancel={() => setShowAddForm(false)}
              />
            ) : (
              <button onClick={() => setShowAddForm(true)}
                style={{
                  background: "none", border: "0.5px dashed var(--border-default)",
                  borderRadius: "var(--radius-lg)", color: "var(--text-muted)",
                  fontSize: 13, padding: "14px 0", cursor: "pointer",
                  display: "flex", alignItems: "center", justifyContent: "center",
                  gap: 6, fontFamily: "inherit", width: "100%",
                }}>
          ...
          <button onClick={handleSave}
            style={{
              marginTop: 20, width: "100%", background: "var(--accent)", border: "none",
              borderRadius: "var(--radius-md)", color: "#fff", fontSize: 15, fontWeight: 500,
      ...
      {showDayPicker && (
        <DayOverridePicker
          current={effectiveDayKey}
          onSelect={handleDayOverride}
